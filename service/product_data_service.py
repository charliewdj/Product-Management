...
from . import common_msg, product_data_repository
from .input_validator import validate_date

logger = logging.getLogger(__name__)


def update_product_data(file_name, user, check_list, new_name, new_eos):
    """
    選択された行のアップデート処理
    """
    ...
    try:
        ...
        with open(file_name, "r") as file:
            ...
            for row in reader:
                ...
                # 検索条件
                if row_count == 0:
                    select_list = row[0], row[1], row[2], row[3], row[4], row[5]
                ...
                if int(check_list[check_count]) == row_count:
                    ...
                    sql = f"""
                        UPDATE
                           part_management_product
                        SET
                           sales_item_name = '{up_name}'
                          ,update_date = '{now_date}'
                          ,update_user_code = '{user}'
                          ,eos = { f"'{up_eos}'" if up_eos is not None else "NULL" }
                        WHERE
                           sales_item_number = '{item_number}'
                    """
    ...
    result, err_msg = product_data_repository.get_product_data(file_name, select_list)
